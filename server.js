...
if (cluster.isPrimary) {
    ...
    // API URLs from .env
    const RAPIDO_ANTHROPIC_URL = process.env.RAPIDO_ANTHROPIC_URL;
    const RAPIDO_GEMINI_URL = process.env.RAPIDO_GEMINI_URL;
    const RAPIDO_OPENAI_URL = process.env.RAPIDO_OPENAI_URL;
    ...
    const handleFinalResponse = async (req, res, stream, model, ask, answer, completionId) => {
        let finalAnswer = answer;
        const lowerAsk = ask.toLowerCase();
        const fileRequestRegex = /(?:en|as|au format|en tant que)\s+(pdf|docx|txt)/;
        const match = lowerAsk.match(fileRequestRegex);
            ...
            try {
                if (format === 'pdf') {
                    ...
                    const doc = new Document({
                        sections: [{ properties: {}, children: [new Paragraph({ text: answer })] }],
                    });
                ...
                finalAnswer = `${answer}\n\nVoici votre document : [${format.toUpperCase()} Fichier](${fileUrl})`;
            } catch (e) {
                console.error(`Failed to generate ${format} file:`, e);
            }
        }

        if (stream) {
            res.setHeader('Content-Type', 'text/event-stream');
            res.write(`data: ${JSON.stringify({ id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model, choices: [{ index: 0, delta: { role: 'assistant' }, finish_reason: null }] })}\n\n`);
            res.write(`data: ${JSON.stringify({ id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model, choices: [{ index: 0, delta: { content: finalAnswer }, finish_reason: null }] })}\n\n`);
      ...
      try {
        ...
        if (fileId) {
            try {
                ...
                if (fileResult.rows.length > 0) {
                    ...
                    // TODO: Implement a more sophisticated chunking mechanism for very large files.
                    // For now, prepend the entire content.
        ...
        if (gpt5Models.includes(model)) {
            ...
            if (triggerKeyword && !imageUrl) {
                ...
                return handleFinalResponse(req, res, stream, model, ask, responseContent, completionId);
            }
            ...
            return handleFinalResponse(req, res, stream, modelUsed, ask, answer, completionId);
        }
        const lowerCaseAsk = ask.toLowerCase().trim();
        const triggerKeyword = imageGenerationKeywords.find(keyword => lowerCaseAsk === keyword || lowerCaseAsk.startsWith(keyword + ' '));
        if (triggerKeyword && !imageUrl) {
        ...
        if (geminiModels.includes(model)) {
            ...
            const completionId = `chatcmpl-${Date.now()}`;
            return handleFinalResponse(req, res, stream, model, ask, answer, completionId);
        }
    ...
    app.post('/v1/images/generations', async (req, res) => {
      ...
      try {
        ...
        const finalImageUrl = freeimageResponse.data.image.url;
    ...
    const server = app.listen(PORT, () => {
      ...
      const requiredVars = [
        'HAJI_API_KEY', 'HAJI_GEMINI_API_KEY', 'HAJI_PUTER_API_KEY', 'HAJI_OPENAI_API_KEY', 'FREEIMAGE_API_KEY',
        'RAPIDO_ANTHROPIC_URL', 'RAPIDO_GEMINI_URL', 'RAPIDO_OPENAI_URL',
        'HAJI_FLUX_URL', 'HAJI_PUTER_URL', 'HAJI_IMAGEN_URL', 'HAJI_POLLINATION_URL', 'HAJI_FLUXWEBUI_URL', 'HAJI_RTM_URL'
      ];
